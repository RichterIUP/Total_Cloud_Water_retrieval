...
def read_radiances(spectral_radiance_file, date_of_spec, sza):
    ...
    if inp.RAD_INP == 'OPUS':
        DATETIME = dt.datetime.strptime(spectral_radiance_file.split("nyem")[-1].split("_")[0], "%Y%m%d%H%M%S")
    
        ## Read radiances and wavenumber from OPUS file
        s = ftsreader.ftsreader(spectral_radiance_file, verbose=True, getspc=True, getifg=True)
...
def create_atmosphere(atmospheric_file):
    ...
    if inp.ATM_INP == "ERA5":
        ...
        relative_humidity = read_era5_argmin(atmospheric_file, LAT, LON, time_era5, inp.KEY_RH, inp.TIME_UNIT)[0]
        height = read_era5_argmin(atmospheric_file, LAT, LON, time_era5, inp.KEY_HEIGHT, inp.TIME_UNIT)[0]/physics.G0 #Convert geopotential to geometric height
        temperature, plev = read_era5_argmin(atmospheric_file, LAT, LON, time_era5, inp.KEY_TEMP, inp.TIME_UNIT)
        ...
        if len(atmospheric_profiles) > 1:
            ...
            for file_ in atmospheric_profiles:
                time_diff.append(np.abs((dt.datetime.strptime(file_, "prof%Y%m%d_%H%M.nc")-DATETIME).total_seconds()))
                time_atm.append((dt.datetime.strptime(file_, "prof%Y%m%d_%H%M.nc")-DATETIME).total_seconds())
        ...
        for ii in range(len(atmospheric_profiles)):
            with nc.Dataset("{}/{}".format(atmospheric_file, atmospheric_profiles[ii])) as atm_f:
                ...
                pressure_atm[ii]    = np.interp(reduced_altitude, altitude_pre_int, pressure_pre_int)
                temperature_atm[ii] = np.interp(reduced_altitude, altitude_pre_int, temperature_pre_int)
                humidity_atm[ii]    = np.interp(reduced_altitude, altitude_pre_int, humidity_pre_int)
                altitude_atm[ii]    = reduced_altitude
        ...
        for ii in range(size_atm):
            height[ii]    = np.interp(0, time_atm[idx], np.array([altitude_atm[0][ii], altitude_atm[1][ii]]))
            plev[ii]    = np.interp(0, time_atm[idx], np.array([pressure_atm[0][ii], pressure_atm[1][ii]]))
            temperature[ii] = np.interp(0, time_atm[idx], np.array([temperature_atm[0][ii], temperature_atm[1][ii]]))
            relative_humidity[ii]    = np.interp(0, time_atm[idx], np.array([humidity_atm[0][ii], humidity_atm[1][ii]]))
        height *= 1e3
...
def read_cloud_position(cloud_height_file):
    ...
    if inp.CLD_INP == "ASCII":
        ...
        date_of_spec_no_sec = DATETIME #dt.datetime(DATETIME.year, DATETIME.month, DATETIME.day, DATETIME.hour, DATETIME.minute)
...
def create_nc(num_iter, index=-1, avk_matrix=None, errors=None, covariance_matrix=None, transfer_matrix=None):
    '''
