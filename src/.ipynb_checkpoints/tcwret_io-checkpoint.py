...
def read_radiances(spectral_radiance_file, date_of_spec, sza):
    ...
    if inp.RECALIBRATE:
        ...
        #emiss_f = lambda x : 0.975

        # Load temperature of laboratory
        temp_lab = pd.read_csv(inp.FILE_TEMP, parse_dates=[1])
        #print(DATETIME)
        #print(np.where((temp_lab['time'] > DATETIME) & (temp_lab['time'] < DATETIME + \
    ...
    low = 1925
    idx_noise = np.where((physics.WAVENUMBER_FTIR > low) & (physics.WAVENUMBER_FTIR < 2000))[0]
    ...
    elif inp.STDDEV_OPT == 'B':
        physics.NOISE_FTIR = np.mean(noise_av)*np.ones(wavenumber_av.size)
